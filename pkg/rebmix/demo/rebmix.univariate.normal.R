...
N[[1]]$w <- 1
N[[1]]$Theta <- rbind(pdf = "normal", theta1 = 0, theta2 = 1)

N[[2]]$density <- "Skewed unimodal"
N[[2]]$w <- c(rep(1/5, 2), 3/5)
N[[2]]$Theta <- rbind(pdf = "normal", theta1 = c(0, 1/2, 13/12), theta2 = c(1, 2/3, 5/9))
...
N[[3]]$Theta <- rbind(pdf = "normal", theta1 = theta1(0:7), theta2 = theta2(0:7))

N[[4]]$density <- "Kurtotic unimodal"
N[[4]]$w <- c(2/3, 1/3)
N[[4]]$Theta <- rbind(pdf = "normal", theta1 = rep(0, 2), theta2 = c(1, 1/10))

N[[5]]$density <- "Outlier"
N[[5]]$w <- c(1/10, 9/10)
N[[5]]$Theta <- rbind(pdf = "normal", theta1 = rep(0, 2), theta2 = c(1, 1/10))

N[[6]]$density <- "Bimodal"
N[[6]]$w <- rep(1/2, 2)
N[[6]]$Theta <- rbind(pdf = "normal", theta1 = c(-1, 1), theta2 = rep(2/3, 2))

N[[7]]$density <- "Separated bimodal"
N[[7]]$w <- rep(1/2, 2)
N[[7]]$Theta <- rbind(pdf = "normal", theta1 = c(-3/2, 3/2), theta2 = rep(1/2, 2))

N[[8]]$density <- "Skewed bimodal"
N[[8]]$w <- c(3/4, 1/4)
N[[8]]$Theta <- rbind(pdf = "normal", theta1 = c(0, 3/2), theta2 = c(1, 1/3))

N[[9]]$density <- "Trimodal"
N[[9]]$w <- c(rep(9/20, 2), 1/10)
N[[9]]$Theta <- rbind(pdf = "normal", theta1 = c(-6/5, 6/5, 0), theta2 = c(rep(3/5, 2), 1/4))
...
N[[10]]$Theta <- rbind(pdf = "normal", theta1 = c(0, theta1(0:4)), theta2 = c(1, rep(1/10, 5)))
...
N[[11]]$Theta <- rbind(pdf = "normal", theta1 = c(-1, 1, theta1(0:6)), theta2 = c(rep(2/3, 2), rep(1/100, 7)))
...
N[[12]]$Theta <- rbind(pdf = "normal", theta1 = c(0, theta1(-2:2)), theta2 = c(1, theta2(-2:2)))
...
N[[13]]$Theta <- rbind(pdf = "normal", theta1 = c(theta1.1(0:1), theta1.2(1:3), theta1.3(1:3)), theta2 = c(rep(2/3, 2), rep(1/100, 3), rep(7/100, 3)))
...
N[[14]]$Theta <- rbind(pdf = "normal", theta1 = theta1(0:5), theta2 = theta2(0:5))
...
N[[15]]$Theta <- rbind(pdf = "normal", theta1 = c(theta1.1(0:2), theta1.2(8:10)), theta2 = c(rep(2/7, 3), rep(1/21, 3)))
