...
gamma
\begin{equation}
\theta_{il} = \frac{\hat{y}_{im}}{\beta_{il} - 1} \textrm{, } f(\beta_{il}) = \varepsilon f_{i | \hat{i}.lm} \hat{y}_{im} - e^{\beta_{il} \log (\beta_{il} - 1) -\beta_{il} + 1 - \log(\Gamma [\beta_{il}])} = 0 \textrm{ and } \beta_{il} > 1,
\end{equation}
binomial
\begin{equation}
...
\begin{figure}[t]
\centering
<<complex2-fig, fig = TRUE, pdf = FALSE, png = TRUE, eps = FALSE, height = 6.5, width = 5.5, echo = false, keep.source = FALSE>>=
## Visualize Figure 4.
