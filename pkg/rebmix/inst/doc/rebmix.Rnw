...
\theta_{il} = \hat{y}_{im}
\end{equation}
component parameters are derived, where $\gamma$ is the Euler-Mascheroni constant. When deriving (\ref{eq:13}) $\Gamma [\beta_{il}]$ is approximated by the Stirling's formula and digamma function by $\psi(\beta_{il}) = \log(\beta_{il}) - \gamma / \beta_{il}$. Rough binomial parameter $\theta_{il} = \theta_{i}$ is fixed and equals the number of categories minus one. The rigid restraints
become loose if $\hat{y}_{im}$ and $f_{i | \hat{i} . l\mathrm{max}}$ of (\ref{eq:5}) to (\ref{eq:8}) are supposed to be bounded by
\begin{equation}
