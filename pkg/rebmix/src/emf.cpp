    ...
    for (l = 0; l < c_; l++) {
        ...
        for (i = 0; i < length_pdf_; i++) {
            ...
            case pfWeibull:
				...
				while ((j <= ItMax) && Error) {
					...
					for (k = 0; k < n_; k++) if (Y_[i][k] > FLOAT_MIN) {
            ...
            case pfGumbel:
				...
				while ((j <= ItMax) && Error) {
					...
					for (k = 0; k < n_; k++) {
						...
						A[1] += P_[l][k] * Y_[i][k];
						A[2] += P_[l][k] * T[0];
						A[3] += P_[l][k] * Y_[i][k] * T[0];
						A[4] += P_[l][k] * Y_[i][k] * Y_[i][k] * T[0];
					}
            ...
            case pfvonMises:
				...
				for (j = 0; j < n_; j++) {
					A[0] += P_[l][j] * (FLOAT)cos(Y_[i][j]);
					A[1] += P_[l][j] * (FLOAT)sin(Y_[i][j]);
				}
        ...
        for (i = 0; i < length_pdf_; i++) {
            ...
            case pfvonMises:
				...
				for (j = 0; j < n_; j++) {
					A[1] += P_[l][j] * (FLOAT)cos(Y_[i][j] - M[i]);
				}
