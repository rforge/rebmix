...
poissonclu <- RCLRMIX(x = poissonest, pos = 9)

plot(poissonclu)
@
\caption{Poisson dataset. Predictive cluster membership (coloured large circles).}
\end{figure}
...
plot(wreathest, pos = 1, what = c("dens"), nrow = 1, ncol = 1)
@
\caption{Dataset \texttt{wreath}. Empirical densities (coloured large circles), predictive multivariate normal mixture density (coloured lines).}
\end{figure}
\subsubsection{Clustering}
\begin{figure}[htbp]\centering
...
plot(wreathclu)
@
\caption{Dataset \texttt{wreath}. Predictive cluster membership (coloured large circles).}
\end{figure}
\subsection{Multivariate normal dataset}
...
@
\caption{Dataset \texttt{ex4.1}. Empirical densities (coloured large circles), predictive multivariate normal mixture density (coloured lines).}
\end{figure}
\subsubsection{Clustering}
\begin{figure}[htbp]\centering
...
plot(ex4.1clu)
@
\caption{Dataset \texttt{ex4.1}. Predictive cluster membership (coloured large circles).}
\end{figure}
