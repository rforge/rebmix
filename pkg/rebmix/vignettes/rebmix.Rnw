...
\section{What's new in version \texttt{2.10.3}}
Methods \texttt{Zp} and \texttt{coef} are replaced by \texttt{a.Zp}, \texttt{a.theta1.all} and \texttt{a.theta2.all} getters. All slots can now be accessed via accessors. Their names are  generally composed of \texttt{a.} followed by the slot name and are used to read the slots. Class \texttt{"RNGMIX.Theta"} is added to simplify random finite mixture model generation. Method \texttt{show} is added for \texttt{"RCLS.chunk"} class. Minimum number of component \texttt{cmin} is added to \texttt{REBMIX} arguments and to the \texttt{"REBMIX"} class.  The \texttt{"Parzen window"} preprocessing is renamed to more well known \texttt{"kernel density estimation"}. Rough parameter estimation for binomial and Poisson parametric families is improved. The package is broadened to latent class analysis in version 2.10.3, too. Method \texttt{split} is improved and examples for its proper use are added. GCC 8.1 notes and warnings in C++ functions are eliminated in version 2.10.2, too. Cholesky decomposition is now used to calculate logarithm of determinant and inverse of variance-covariance matrices instead of LU decomposition. Special attention is paid to resolving numerical problems related to high dimensional datasets. Version 2.10.1 is the further debugged version 2.10.0. Large \texttt{K} in combination with large dimension $d$ can lead to histograms with numerous nonempty bins $v$. In order to restrain $v$, the well known RootN rule \citep{Velleman_1976} may intuitively be extended to multidimensions
\begin{equation}
...
The author thanks Branislav Pani\'{c} for his contribution to the \citet{Velleman_1976} rule extension to multidimensions and for his other valuable suggestions on the package from version 2.9.0 on.
