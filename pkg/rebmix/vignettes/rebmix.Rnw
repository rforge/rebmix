...
Version \texttt{2.13.0} introduces several major improvements of the package. Gumbel distribution has beed added to the parametric familty types. Gumbel mixture models can now be estimated. Let $y$ be a random variable following the Gumbel distribution. The probability density function of $y$ has the following form:

\begin{equation}
f(y) = \frac{1}{\beta}\exp\left(-\frac{y-\mu}{\beta} - \exp\left(-\frac{x-\mu}{\beta}\right)\right).
\end{equation}

Therefore, in mixture model setting paremeters $\mu$ and $\beta$ are estimated for each component of the mixture model. For this the REBMIX and EM algorithm are derived.

EM algorithm has been implemented for all of the parametric family types (normal, lognormal, Weibull, gamma, binomial, Poisson, Dirac or von Mises parametric families). Therefore, EM algorithm can now be used with all of the parametric family types. Additionally, histogram based EM algorithm has beed added to speed up the EM algorithm, when the dataset contains large numbers of discrete observations. 

Finaly, the package was further debugged and some of the core functions are improved.   
...
A \texttt{mvnorm} dataset from \citep{Panic_2020c} consist of $250$ observations drawn from a $5$-component normal mixture.
...
\subsubsection{Plot method}
\begin{figure}[htb]\centering
<<mvnorm-fig, fig = TRUE, pdf = TRUE, png = FALSE, eps = FALSE, height = 5.0, width = 5.5, echo = TRUE, results = hide, keep.source = FALSE>>=
plot(mvnormest)
@
...
\subsubsection{Clustering}
\begin{figure}[htb]\centering
<<mvnorm-clu-fig, fig = TRUE, pdf = TRUE, png = FALSE, eps = FALSE, height = 4.5, width = 5.5, echo = TRUE, results = hide, keep.source = FALSE>>=
...
Single REBMIX\&EM strategy requires the preprocessing parameter \texttt{K} to be known. This parameter can be estimated using the \texttt{optbins} method. To estimate the optimal parameter \texttt{K} different rules are implemented. More information can be found in \citep{Panic_2020c}.
...
@
\begin{figure}[htb]\centering
<<mvnorm-clu-fig-cem, fig = TRUE, pdf = TRUE, png = FALSE, eps = FALSE, height = 5.5, width = 5.5, echo = TRUE, results = hide, keep.source = FALSE>>=
plot(mvnorm.clu)
@
\caption{Dataset \texttt{mvnorm}. Strategy used: exhaustive REBMIX\&ECM strategy. Criterion: ICL.  Predictive cluster membership (coloured circles).}
...
\subsection{Multivariate \texttt{sensorless.drive} dataset}
These data are the results of sensorless drive diagnosis procedure. Features are extracted from electric current drive signals \citep{sensorless_drive_citation}. Main objective is the sensorless fault detection and classification. 
<<rebmix-code, split = FALSE, keep.source = FALSE>>=
...
\subsubsection{Plot method}
\begin{figure}[htb]\centering
<<drive-cla-fig, fig = TRUE, pdf = TRUE, png = FALSE, eps = FALSE, height = 5.5, width = 5.5, echo = TRUE, results = hide, keep.source = FALSE>>=
...
\subsubsection{Estimation of using the histogram based EM}

When dealing with large datasets it is useful to shrink datasets to reduce the computational overload with the EM algorithm. The histogram based EM algorithm can be used for this. The dataset is discretized and EM is computed with the histogram data. Integer parameter \texttt{K} from the \texttt{EM.Control} object is used to set the shrinkage level. Smaller value means less containers while larger implicates more containers.
